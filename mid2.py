...
def get_color(height):
    if height < 200:
        return (255, 255, 255)  # белый для высоких точек
    elif height < 400:
        return (139, 69, 19)  # коричневый для средних точек
    else:
        return (34, 139, 34)  # зеленый для низких точек


def renderHeights(screen, heights, zoom, vertical_shift):
    for i in range(len(heights) - 1):
        x1 = int(i * zoom)
        y1 = int((heights[i] - vertical_shift) * zoom)
        x2 = int((i + 1) * zoom)
        y2 = int((heights[i + 1] - vertical_shift) * zoom)
        if 0 <= y1 < 600 and 0 <= y2 < 600:
            color = get_color((heights[i] + heights[i + 1]) // 2)
